...
load("./data/partition_SNPdata_forSimulation.RData") # part.mat - here we load a matrix, rather than a vector
...
Now put together the matrix with effects `B`, here denoted by object `Bl` to refer to the larger number of exons (50).
...
snp.eff[[ 2 ]] <- snp.eff[[ 1 ]]
save(snp.eff, file=paste("./output/snp_effects_forSimulation.RData", sep="")) # snp.eff
...
The same can be repeated using 3 exons. Below is the chunk to load the simulated matrix of effects, to be run separately.

```{r def.load.objects.effects.3exons, eval=F}
# This can only be run when Bs is  created
myname <- "N10000_linkF_equalSetSizes"
#source("load_objects_forSimulation_20_2_equalSetSizes.R")
# In script assign_effects.R: snp_eff are created and saved
# Here for 50 exons
nexons <- 3
prop.aff <- c(0.02, 0.02, 0.20) # proportion of SNPs with an effect for the 3 situations
eff.size <- c(5, 1, 1) # effect size, or beta values to be considered, for the 3 situations
...
load("./data/Bmatrix_smallNumberExons.RData") # Bs
```


### Simulating data and testing
...
Here we generate the responses, and compute the test statistics p-values using both all samples
 as well as using all subsets.
...
load("./output/pvalues_effectAndNull_normAndMultin.I.RData") # pvals.part, pvals.part.0
```



### Test using the normal and multinomial
...
P-values are then computed for the entire data, as well for partitions. We consider the normal and the multinomial (ignoring the correlation structure between the SNPs) distributions, both for the cases involving 50% of samples with effect of SNPs on exon expression levels, as well as cases without effect. 

```{r def.reshape.pvals.mats.save}
...
Now use `W` computed from the data. 
...
#save(pvals.part, pvals.part.0, file = "./output/pvalues_effectAndNull_Multin.W.RData")
```

```{r eval = TRUE, echo = FALSE}
load("./output/pvalues_effectAndNull_Multin.W.RData") # pvals.part, pvals.part.0
...
```{r dirs.for.plot.below, eval = FALSE, echo = FALSE}
myname <- "N10000_linkF_equalSetSizes_noStd_SNPIandIItheSame_2genes_diffEffects_10thExAff" # in this case we do not standardize effects
myname2 <- "_largerNExAff_noStd_SNPIandIItheSame_diffEffects_10thExAff"
myname1 <- "N10000_linkF_equalSetSizes_W_noStd_SNPIandIItheSame_2genes_diffEffects_10thExAff"
mywdir <- "~/Documents/NKI/projects/spliceQTL/scripts server"
load(paste(mywdir, "/analysis_multin_WandI_", myname, "_", myname2, ".RData", sep=""))
```

```{r eval = FALSE, echo = FALSE}
# Objects below were previously loaded, but we here load them again for consistency
mydir.output <- "/media/renee/Seagate Expansion Drive/Projects/altSplicing/output simul 2017" 
rm(pvals.part.joint)
load(paste(mydir.output, "/pvals_part_joint", myname, "_", myname2, ".RData", sep="")) # pvals.part.joint
pvals.mat.i <- pvals.part.joint

load(paste(mydir.output, "/pvals_part_joint", myname1, "_", myname2, ".RData", sep="")) # pvals.part.joint
pvals.mat.w <- pvals.part.joint
```
...
For example 1 in the manuscript, We use linear regression for pairwise testing.  Below we include a code chunk where this can be run, using  exon-level data as well as SNP data.  Required objects to run it are:
...
More details about the input required can be found in file `f_spliceQTLfit.R`. 
